...
# ========== 新增：本地已存在则跳过下载 ==========
HIGRESS_LOCAL_TAR="$DESTINATION/higress-${VERSION}.tar.gz"

checkLocalPackage() {
  if [ -f "$HIGRESS_LOCAL_TAR" ]; then
    echo "Found local package $HIGRESS_LOCAL_TAR , skip downloading."
    return 0
  else
    return 1
  fi
...
downloadIfNeeded() {
  if checkLocalPackage; then
    HIGRESS_TMP_FILE="$HIGRESS_LOCAL_TAR"
  else
    download
  fi
}
