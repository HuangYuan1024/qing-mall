...
func parseConfig(json gjson.Result, config *RedisCacheConfig, log wrapper.Log) error {
	...
	config.ttl = int64(json.Get("ttl").Int())
...
func onRequestHeaders(ctx wrapper.HttpContext, config RedisCacheConfig, log wrapper.Log) types.Action {
	...
	if err != nil {
		...
		log.Errorf("Redis配置: serviceName=%s, servicePort=%d, ttl=%d, timeout=%d", config.serviceName, config.servicePort, config.ttl, config.timeout)
		return types.ActionContinue
	}
...
func onResponseHeaders(ctx wrapper.HttpContext, config RedisCacheConfig, log wrapper.Log) types.Action {
	...
	if err != nil || status != "200" {
		...
		return types.ActionContinue
	} else if _, err := proxywasm.GetHttpResponseHeader("redis-cache"); err == nil {
		log.Debugf("响应头已包含缓存标记，跳过缓存: %s", cacheKey)
		// 清除缓存键，避免在响应体阶段处理
		ctx.SetContext("cacheKey", "")
		return types.ActionContinue
	} else {
		// 添加缓存标记头
		proxywasm.AddHttpResponseHeader("redis-cache", "MISS")
		proxywasm.AddHttpResponseHeader("redis-cache-key", cacheKey)
...
func onResponseBody(ctx wrapper.HttpContext, config RedisCacheConfig, body []byte, log wrapper.Log) types.Action {
	...
	err := config.client.Set(cacheKey, string(body), func(response resp.Value) {
		if response.Error() != nil {
			...
			// 设置过期时间
			err := config.client.Expire(cacheKey, int(config.ttl), func(response resp.Value) {
				if response.Error() != nil {
